...
app = Flask(__name__)
app.config["SECRET_KEY"] = os.environ.get("BLOODLINK_SECRET_KEY", "dev-only-change-this-secret")
...
def init_db(seed_demo=True):
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_donors_blood_group ON donors(blood_group)"); conn.execute("CREATE INDEX IF NOT EXISTS idx_interactions_request ON donor_interactions(request_id)"); conn.commit(); conn.close()
...
def login_required(role=None):
    def decorator(fn):
        ...
        def wrapped(*args, **kwargs):
            ...
            if role and user.get("role") not in (role if isinstance(role, tuple) else (role,)):
                return error("You do not have permission for this action.", "FORBIDDEN", 403)
...
def match_donors():
    ...
    conn=get_db(); cur=conn.execute("INSERT INTO blood_requests (blood_group,latitude,longitude,max_distance_km,urgency) VALUES (?,?,?,?,?)",(bg,lat,lon,radius,urgency)); result["request_id"]=cur.lastrowid; conn.commit(); conn.close()
